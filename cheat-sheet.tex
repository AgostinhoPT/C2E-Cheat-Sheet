...
\subsection{Resolução Analítica de EDOs de 1.ª Ordem}
...
\begin{theorembox}{EDO de Variáveis Separáveis}
  ...
  \textbf{Exemplo Prático (Slide 1):} $y' - 3y = 0 \implies \frac{1}{y} \dd y = 3 \dd x \implies \ln|y| = 3x + C \implies y(x) = \pm e^C e^{3x} = C_0 e^{3x}$ (com $C_0 \neq 0$). Contudo, $y(x) = 0$ é também solução da EDO original, tendo sido perdida ao dividir por $y$. Esta é uma solução singular (não incluída na geral para $C_0 \neq 0$, a menos que $C_0 \in \R$).
\end{theorembox}
...
\begin{formulabox}{Modelos Exponenciais}
  ...
  \begin{itemize}
    ...
    \item \textbf{Meia-vida ($T_{1/2}$):} Tempo para decair a metade: $y(T_{1/2}) = \frac{1}{2}y(0) \implies k = - \frac{\ln 2}{T_{1/2}}$.
  \end{itemize}
